...
class ProduitController extends AbstractController
{
    ...
    public function new(Request $request, ProduitRepository $produitRepository): Response
    {
        ...
        if($form->isSubmitted() && $form->isValid()){
            //on verifie q'un fichier a ete televerse
            $fichier = $form->get("photo")->getData();
            
            if($fichier){
                //on recupere le nom du fichier
                $nomFichier = pathinfo($fichier->getClientOriginalName(),PATHINFO_FILENAME);
                //La classe AsciiSlugger va remplacer les caracteres speciaux paar des caracteres autorises dans les URL
                $slugger = new AsciiSlugger();
                $nomFichier = $slugger->slug($nomFichier);
                //La fonction PHP 'uniquid va generer un string sur le serveur
                $nomFichier .= "_" . uniqid();
